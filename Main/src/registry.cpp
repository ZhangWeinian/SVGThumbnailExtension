...
STDAPI CreateRegistryKey(REGKEY_SUBKEY_AND_VALUE* pKey)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (LSTATUS status {
				SHSetValue(pKey->hKey, pKey->lpszSubKey, pKey->lpszValue, pKey->dwType, pvData, (DWORD)cbData) };
			NOERROR != status)
		{
...
STDAPI DeleteRegistryKeys(REGKEY_SUBKEY* aKeys, ULONG cKeys)
{
	...
	for (ULONG iKey { 0 }; iKey < cKeys; iKey++)
	{
		...
		if (LSTATUS status = RegDeleteTree(aKeys[iKey].hKey, aKeys[iKey].lpszSubKey); NOERROR != status)
		{
			hr = HRESULT_FROM_WIN32(status);
